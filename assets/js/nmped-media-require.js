jQuery(document).ready(function ($) {
	...
	var checkForAlt = function (showNotice) {
		...
		$('.nmped-needs-alt-text').each(function (idx, li) {
			$(li).removeClass('nmped-needs-alt-text');
		});
		...
		if (0 === selectedImages.length) { // This is seen in some modals.
			...
			if (0 === $('.media-sidebar.visible').length || ( altText.length && 0 < altText.length )) {
				...
				$parent.addClass('nmped-has-alt-text');
			...
			$parent.removeClass('nmped-has-alt-text');

			if (notice) {
				alert(nmpedTagsCopy.editTxt);
			}
			...
			selectedImages.each(function (idx, li) {
				...
				if ('undefined' !== typeof imageId) { // It's not actually an image or even an uploaded item.
					...
					if (altText.length || 'image' !== image.get('type')) { //looks like we're OK on this one.
						...
						$parent.addClass('nmped-has-alt-text');
						$image.removeClass('nmped-needs-alt-text');

					} else { // Mark it 0 dude.

						$image.addClass('nmped-needs-alt-text');
			...
			if (false === canProceed) {
				...
				if (notice) {
					...
					alert('\n' + nmpedTagsCopy.editTxt + ':' + imageList);
